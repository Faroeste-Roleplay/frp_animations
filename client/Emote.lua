...
Citizen.CreateThread(function()
    LocalPlayer.state:set('canClearAnim', true)
    ...
    while true do
        ...
        if IsControlJustPressed(0, `INPUT_FRONTEND_CANCEL`) and not buttonPressed then
            EmoteCancel()
            buttonPressed = true
            Wait(100)
        end
...
AddEventHandler('animations:client:EmoteCommandStart', function(args)
    if CanDoEmote and not IsPedInAnyVehicle(PlayerPedId(), false) then
        EmoteCommandStart(args)
    else
        Core.Functions.Notify("Você não pode fazer emote neste momento", "error")
    end
end)
...
CheckGender = function()
    local hashSkinMale = GetHashKey("mp_male")
    local hashSkinFemale = GetHashKey("mp_femalr")
    ...
    if ChosenDict == "MaleScenario" or "Scenario" then
        ...
        if ChosenDict == "MaleScenario" then
            if PlayerGender == "male" then
                ClearPedTasks(PlayerPedId())
                TaskStartScenarioInPlace(PlayerPedId(), GetHashKey(ChosenAnimation), -1, true, 0, 0, 0);
